...
<div>
  ...
  <h2>ROCKET: {$consensusTipState.RocketMap.get($page.params.id)?.Name.toLocaleUpperCase()}</h2>
  <Row>
    <Column sm={16} md={16} lg={16} max={8}><AspectRatio ratio="2x1" style="margin:1%;"><Tile style="height:100%; width:100%;">
      <CodeSnippet type="multi" code={JSON.stringify($consensusTipState.RocketMap.get($page.params.id)?.Event.rawEvent(), null, '\t')} expanded/>
    </Tile></AspectRatio></Column>
    <Column sm={16} md={16} lg={16} max={8}><AspectRatio ratio="2x1" style="margin:1%;"><Tile style="height:100%; width:100%;"><h3>About</h3><p>Created by: {$consensusTipState.RocketMap.get($page.params.id)?.CreatedBy}</p>Problem it's solving, mission, description etc</Tile></AspectRatio></Column>
    ...
    <Column sm={16} md={16} lg={16} max={8}><AspectRatio ratio="2x1" style="margin:1%"><Tile style="height:100%; width:100%;"><h3>{$consensusTipState.RocketMap.get($page.params.id)?.Name} Products</h3>The latest x products that can be purchased from this rocket</Tile></AspectRatio></Column>
