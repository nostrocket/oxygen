...
{#if button}
  ...
  >
    {#if parent?.UID.length === 64}Log a sub-problem{:else if existing}Edit this
      problem{:else}New Problem Now{/if}
  </Button>
{:else}
  <a
    style="padding: 15px 15px; font-size: 14px; color: aliceblue"
    on:click={() => {
      formOpen = true;
    }}
  >
